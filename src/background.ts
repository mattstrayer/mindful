...
chrome.tabs.onUpdated.addListener(
  function (tabId, changeInfo, tab) {
    ...
    if(tab.url) {
      const shouldBlock = DomainBlockingService.shouldBlockDomain(tab.url) ? `should block ${tab.url}` : ''

      console.log(shouldBlock)
    }
    if (changeInfo.url) {

      if(changeInfo.url.includes(chrome.runtime.getURL("tabs/index.html"))) return


      const shouldBlock = DomainBlockingService.shouldBlockDomain(changeInfo.url) ? `should block ${changeInfo.url}` : ''
